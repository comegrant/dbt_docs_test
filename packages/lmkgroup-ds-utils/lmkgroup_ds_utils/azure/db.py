...
class DatabaseSparkConnector:
        ...
        if self.db == "mldb":
            return (
                ...
                self.dbutils.secrets.get(
                    scope="DataBricksScope", key="MLAnalyticsDBPassword"
                ),
            )
            ...
            return (
                conf["read_db"],
                self.dbutils.secrets.get(
                    scope="DataBricksScope", key="AnalyticsDBPassword"
                ),
            )
        elif self.db == "postgresdb":
            return (
                ...
                self.dbutils.secrets.get(
                    scope="DataBricksScope", key="PostgresDBPassword"
                ),
            )
    ...
    def get_connection_properties(self):
        ...
        if self.db != "postgresdb":
            self.jdbcUrl = "jdbc:sqlserver://{0}:{1};database={2}".format(
                jdbc_hostname, jdbc_port, jdbc_database
            )
        else:
            self.jdbcUrl = "jdbc:postgresql://{0}:{1}/{2}".format(
                jdbc_hostname, jdbc_port, jdbc_database
            )
    ...
    def extract_from_db(self, query):
        ...
        return self.spark.read.jdbc(
            self.jdbcUrl, query, properties=self.connection_properties
        )
    ...
    def delete_from_table(self, ids, schema, table_name):
        ...
        try:
            ...
            logger.exception(
                "Failed to delete from table %s.%s: %s", schema, table_name, e
            )
    ...
    def insert_to_db(
        ...
        try:
            ...
            if count > 100000:
                df.write.format("com.microsoft.sqlserver.jdbc.spark").mode(
                    "append"
                ).option("url", self.jdbcUrl).option(
                    "dbtable", f"{schema}.{table_name}"
                ).option(
                    "username", self.connection_properties["user"]
                ).option(
            ...
            logger.exception(
                "Failed to write to table %s.%s: %s", schema, table_name, e
            )
