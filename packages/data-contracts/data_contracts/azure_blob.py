...
{self.config.to_markdown}"""
    ...
    def multi_source_features_for(  # type: ignore
        cls, # noqa: ANN102
        facts: RetrivalJob, requests: list[tuple[AzureBlobCsvDataSource, RetrivalRequest]]
    ) -> RetrivalJob:
...
{self.config.to_markdown}"""
    ...
    async def write_polars(self, df: pl.LazyFrame, request: RetrivalRequest | None = None) -> None:
    ...
    def multi_source_features_for(  # type: ignore
        cls, # noqa: ANN102
        facts: RetrivalJob, requests: list[tuple[AzureBlobParquetDataSource, RetrivalRequest]]
    ) -> RetrivalJob:
    ...
    async def upsert(self, job: RetrivalJob, request: RetrivalRequest) -> None:
        ...
        for row in unique_partitions.iter_rows(named=True):
            ...
            for partition_key in self.partition_keys:
                dir_path  = dir_path  / f"{partition_key}={row[partition_key]}"

            if fs.exists(dir_path .as_posix()):
                delete_directory_recursively(dir_path .as_posix())
...
{self.config.to_markdown}"""
    ...
    def multi_source_features_for(  # type: ignore
        cls, # noqa: ANN102
        facts: RetrivalJob, requests: list[tuple[AzureBlobDeltaDataSource, RetrivalRequest]]
    ) -> RetrivalJob:
