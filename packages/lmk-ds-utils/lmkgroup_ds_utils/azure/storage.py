...
class BlobConnector:
    ...
    def upload_df(self, container: str, dataframe: pd.DataFrame, file_path: str, filename: str):
        """
...
class BlobMount:
            ...
            self.dbutils.fs.mount(
                ...
                extra_configs={
                    "fs.azure.account.key.{storage_account}.blob.core.windows.net".format(
                        storage_account=storage_account,
                    ): self.dbutils.secrets.get(scope=scope, key=secret),
                },
            )
    ...
    def copy_file(self, file: str, path: str):
        ...
        randFolderName = "".join(random.choice(string.ascii_lowercase) for i in range(5))
