...
class DB:
    ...
    def _load_connection_string(self, password: str | None = None):
        if not password:
    ...
    def _create_postgres_engine(self, conn_str):
        ...
        logger.info("Connecting to database %s using conn_str %s", self.db_name, conn_str)
        return create_engine(URL.create("postgresql+psycopg2", query={"odbc_connect": conn_str}))

    def _create_spark_engine(self, conf_db, password):
        """Create the connection properties for spark"""
        jdbc_url = (
            f"jdbc:sqlserver://{conf_db['host']}:{conf_db['port']};database={conf_db['dbname']}"
        )
