...
def run_preselector_engine(
    ...
    num_dishes = customer["variation_meals"]
...
def run_filter_rules(
    ...
    """
    if run_config["filter"]["portion_size"]:
        possible_dishes, debug_summary = filter_portion_size(
            possible_dishes, customer, debug_summary
        )

    if run_config["filter"]["taste_restrictions"]:
        possible_dishes, debug_summary = filter_taste_restrictions(
            possible_dishes, customer, debug_summary
        )
...
def run_basket_rules(
    ...
    df_preference_rules_for_product = df_preference_rules[
        df_preference_rules["product_id"] == customer["product_id"]
    ]

    # Get the first combination
    start_time = time.time()
    basket_combinations = generate_basket_combinations(
        possible_dishes, customer["variation_meals"], run_config
    )
    ...
    while not (all_rules_fulfilled or timelimit_reached):
        ...
        all_rules_fulfilled = True if total_rules_broken == 0 else False
