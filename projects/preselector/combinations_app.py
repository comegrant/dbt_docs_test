...
async def main() -> None:
    ...
    with st.form("Attributes"):
        ...
        selected_attributes = st.multiselect(
            "Attributes",
            options=attributes,
            format_func=lambda att: att.name
        )
        ...
        week = st.number_input(
            ...
            min_value=1,
            max_value=52,
            value=today.isocalendar().week + 2
        )
    ...
    for atters in all_combinations(selected_attributes, include_empty_set=False):
        for prefs in all_combinations(selected_prefs, include_empty_set=True):
            ...
            if response.success:
                ...
                st.error(
                    response.failures[0].error_message
                )
