    ...
    example_output = PreselectorSuccessfulResponse(
        ...
        year_weeks=[
            PreselectorYearWeekResponse(
                ...
                recipe_data=[
                    PreselectorRecipeResponse(
                        main_recipe_id=1, variation_id="abc", compliancy=PreselectorPreferenceCompliancy.all_compliant
                    ),
                    PreselectorRecipeResponse(
                        main_recipe_id=2, variation_id="bca", compliancy=PreselectorPreferenceCompliancy.all_compliant
                    ),
                ],
            )
        ],
        ...
        taste_preferences=[NegativePreference(preference_id="abc", is_allergy=True)],
...
async def test_write_to_preselector_batch_output() -> None:
    request = ForecastedMealkits.query().request
    source = InMemorySource.from_values({feat: [] for feat in request.all_returned_columns})

    store = preselector_store().dummy_store()
    store = store.update_source_for(ForecastedMealkits, source)
    ...
    example_output = PreselectorSuccessfulResponse(
        ...
        year_weeks=[
            PreselectorYearWeekResponse(
                ...
                week=50,
                target_cost_of_food_per_recipe=39,
                error_vector={feat: 0.83 for feat in features[:10]},
                recipe_data=[
                    PreselectorRecipeResponse(
                        main_recipe_id=1, variation_id="abc", compliancy=PreselectorPreferenceCompliancy.all_compliant
                    ),
            ...
            PreselectorYearWeekResponse(
                ...
                week=51,
                target_cost_of_food_per_recipe=39,
                error_vector={feat: 0.83 for feat in features},
                ordered_weeks_ago={
                    1: 202453,
                    2: 202442,
                },
