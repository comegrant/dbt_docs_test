...
from customer_churn.utils.cf_matrix import make_confusion_matrix
...
def baseline_pred(
    df_d: pd.DataFrame,
    num_weeks_without_delivery: int,
    threshold: float,
    customer_since_weeks: int = 52,
    ...
    df_d["pred_churned"] = np.where(
        (df_d["weeks_since_last_delivery"] >= num_weeks_without_delivery),
        1,
        0,
    )
    ...
    rule_2_mask = rule_2_mask.intersection(c)
    df_d.loc[rule_2_mask, "pred_churned"] += 1
...
if __name__ == "__main__":
    # Setup
    num_weeks_without_delivery = 2
    threshold = 0.40

    # Read all files in snapshot folder
    files = Path.glob("../data/processed/snapshots/*.csv")
    df_list = []
    for f in files:
        csv = pd.read_csv(f)
        df_list.append(csv)
    ...
    cf_list = []
    for df in df_list:
        report, cf = baseline_pred(df, num_weeks_without_delivery, threshold)
    ...
    logger.info(f"Churned {np.average(precision_churned)}")
    logger.info(f"Not Churned {np.average(precision_not_churned)}")
    logger.info("Recall")
    logger.info(f"Churned {np.average(recall_churned)}")
    logger.info(f"Not Churned {np.average(recall_not_churned)}")
    logger.info("F1-score")
    logger.info(f"Churned {np.average(f1_score_churned)}")
    logger.info(f"Not Churned {np.average(f1_score_not_churned)}")
