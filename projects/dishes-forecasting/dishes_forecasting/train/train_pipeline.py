...
def train_model(
    ...
    with mlflow.start_run(run_name=run_name):
        ...
        custom_pipeline = define_ensemble_model(
            company_code=company_code, params_lgb=params_lgb, params_xgb=params_xgb, params_rf=params_rf
        )

        training_set_df = training_set.dropna()
        # Coercing datatype
        ...
        df_test_binned.columns = ["_".join(col).strip() for col in df_test_binned.columns.values]
        ...
        registered_model_name = f"{env}.{model_container_name}.{model_name}"
...
def split_train_test(
    ...
    X = training_set.drop(  # noqa
        ["variation_ratio"], errors="ignore", axis=1
    )
