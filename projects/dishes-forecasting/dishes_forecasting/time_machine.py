# Functions to be moved to time machine library
...
def get_date_from_year_week(
    ...
    return pd.to_datetime(weekday_date.date())
...
def get_forecast_year_weeks(
    ...
    df.columns = ["mondays"]
    df_forecast_year_weeks = df["mondays"].dt.isocalendar()[["year", "week"]]

    return df_forecast_year_weeks
