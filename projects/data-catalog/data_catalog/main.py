...
# from aligned.compiler.model import ModelEvaluation, uuid4
...
#from aligned.schemas.model import FeatureInputVersions, FigureMetric, PredictionsView, SingleMetric
from aligned.schemas.model import FeatureInputVersions, PredictionsView
from aligned.schemas.feature import Constraint, EventTimestamp, Feature, FeatureLocation, FeatureReferance, FeatureType
from aligned.schemas.model import ClassificationTarget, RegressionTarget
import pandas as pd
import polars as pl
# from matplotlib.figure import Figure
import asyncio
from dotenv import load_dotenv
# from transaction_ai.model_register import LocalModelRegister, ModelRegister

# from transaction_ai.model_wrapper import F1Macro, F1Weighted, ModelWrapper, PrecisionMacro, PrecisionWeighted, RecallMacro, RecallWeighted, SingleMetricEvaluation, MeanAbsoluteError, RootMeanSquaredError, MeanSquareError, MeanPrecentageError
# from transaction_ai.train import explode_input
...
async def show_model_contract(store: FeatureStore, contract_name: str):
    ...
    with main_col:
        with st.expander("Model Prediction Schema", expanded=True):
            ...
            st.write("To uniquely identify a row in the view, you need to provide the following entities")
            for entity in pred_view.entities:
                feature_cell(entity)

            if pred_view.event_timestamp:
                st.subheader("Event Timestamp")
                st.write("The timestamp describing when the feature was valid from. This can be set to travel back in time. This will also be used to compute the freshness of a view.")
        ...
        with view_predictions:
            ...
            if st.button("Load Predictions"):
                with st.spinner("Loading..."):
                    predictions = (await contract_store.all_predictions(limit=int(limit))
                        .to_polars()
                    ).collect()
        ...
        with evaluate_models:
            ...
            for entity in needed_entities:
                if entity.name not in needed_entitie_names:
                    st.warning(f"Entities `{needed_entitie_names}` is needed, but some are missing in the current prediction view schema.")
                    return
...
async def evaluate_model_contract(
    ...
    if pred_view.event_timestamp:
        entities_names.append(
            f"selected_{pred_view.event_timestamp.name}"
        )

    target_ref = target.estimating

    st.write("#### How it works")
    st.write("""Evaluation will be done by loading all the predictions in the model contract (aka. the `prediction_source`.
...
If the model contract have a `model_version_column` set, then we will evaluate each model version separately.
    ...
    if isinstance(target, ClassificationTarget):
        ...
        baseline_models = [
            MeanPerGroupModel.train_constructor(
                group_by=entities,
                y_column=target.estimating.name
            ),
    ...
    #     preds_job = contract_store.all_predictions()
    #     preds = (await preds_job.to_polars()).collect()
    #
    ...
    #
    #     for model in models:
    #         st.subheader(f"Baseline Model {type(model).__name__}")
    #         preds_column = pl.Series(await model.predict(preds))
    #         await evaluate_model(
    #             RetrivalJob.from_convertable(
    #                 preds.with_columns(
    #                     preds_column.alias(target.feature.name)
    #
    #                 ),
    ...
    with st.spinner("Loading predictions..."):
        preds = (
            await contract_store.all_predictions().to_polars()
        )
    ...
    with st.spinner("Loading targets..."):
        target_job = (store.features_for(
                ...
                event_timestamp_column=f"selected_{pred_view.event_timestamp.name}" if pred_view.event_timestamp else None
            ).log_each_job()
        )
        targets = (await target_job.to_polars()
        ).sort(entities_names).collect()
    ...
    preds = preds.filter(
        pl.concat_str(entities).is_in(
            targets.select(pl.concat_str(entities).alias("key"))["key"]
        )
    )
    ...
    if pred_view.model_version_column:
        ...
        for index, model_version in enumerate(model_versions):
            ...
            with col.expander(f"{model_version}"):
                ...
                #         preds[model_preds][target.estimating.name],
                #         targets[model_preds][target_ref.name]
                #     )
                # else:
                #     baseline_metrics = evaluate_classification_view(
                #         preds[model_preds][target.feature.name],
                #         targets[model_preds][target_ref.name],
                #         col=col
                #     )
                #

        # st.subheader("Most disagreed predictions")
        # disagreement = preds.groupby([
        #     ent.name for ent in pred_view.entities
        # ]).agg([
        #     pl.col(target.feature.name).alias(target.feature.name),
        # ]).with_columns(
        #     unique_predictions=pl.col(target.feature.name).list.unique().list.len(),
        #     total_predictions=pl.col(target.feature.name).list.len()
        # ).with_columns(
        #     disagreement_ratio=pl.col("unique_predictions") / pl.col("total_predictions")
        # ).sort(
        #     "disagreement_ratio", descending=True
        # ).head(100)
        # st.dataframe(disagreement.to_pandas())


    # else:
    #     if isinstance(target, RegressionTarget):
    #         evaluate_regression_view(
    #             preds[target.feature.name],
    #             targets[target_ref.name]
    #         )
    #     else:
    #         baseline_metrics = evaluate_classification_view(
    #             preds[target.feature.name],
    ...
    for index, model in enumerate(models):
        ...
        #         preds,
        #         baseline_target[target_ref.name],
        #         col=col,
...
class MeanPerGroupModel(BaselineModel):
    ...
    @staticmethod
    def train_constructor(group_by: list[str], y_column: str) -> Callable[[pl.DataFrame], Coroutine[None, None, BaselineModel]]:
        async def train(entities: pl.DataFrame) -> BaselineModel:
            return await MeanPerGroupModel.train(entities, y_column, group_by)
        return train

    @staticmethod
    async def train(entities: pl.DataFrame, y_column: str, group_by: list[str]) -> MeanPerGroupModel:
        mean_per_group = entities.to_pandas().groupby(group_by).mean().reset_index()[group_by + [y_column]]
...
class ColumnWrapper(BaselineModel):
    ...
    async def predict(self, X: pd.DataFrame) -> pd.Series:
        ...
        return (await self.store.features_for(X, [self.feature_ref]).to_pandas())[column].fillna("Unknown")


    @staticmethod
    def train_constructor(store: FeatureStore, feature_ref: str) -> Callable[[pl.DataFrame], Coroutine[None, None, BaselineModel]]:
        async def train(entities: pl.DataFrame) -> BaselineModel:
...
class MostCommonModel(BaselineModel):
    ...
    @staticmethod
    def train_constructor(y_column: str) -> Callable[[pl.DataFrame], Coroutine[None, None, BaselineModel]]:
        async def train(entities: pl.DataFrame) -> BaselineModel:
...
class ClassProbabilityModel(BaselineModel):
    ...
    @staticmethod
    def train_constructor(y_column: str, top_n: int | None) -> Callable[[pl.DataFrame], Coroutine[None, None, BaselineModel]]:
        async def train(entities: pl.DataFrame) -> BaselineModel:
    ...
    async def predict(self, x: pd.DataFrame) -> pd.Series:
        ...
        return pd.Series(random.choices(
...
class MeanModel(BaselineModel):
    ...
    @staticmethod
    def train_constructor(entity_names: list[str], y_column: str) -> Callable[[pl.DataFrame], Coroutine[None, None, BaselineModel]]:
...
async def train_baseline_model(
    ...
    with st.spinner("Loading data..."):
        data = (await dataset.train_dataset.all(
            result=result
        ).to_polars()).collect()
...
async def test_transform_for(view: FeatureViewStore):
    ...
    if await use_data_managment_features():
        st.warning("Data managment features are not enabled. Please enable them to use this feature.")
        return

    with st.form("Features To Transform"):
        feature_refs = [
            feat for feat in view.request.all_returned_columns
            if (feat not in view.request.feature_names) and (feat not in view.request.entity_names)
        ]
        features = st.multiselect(
            "Features", feature_refs, default=feature_refs
        )
    ...
    input_features = {
        ref_for_feature(feature)
        for feature in request.entities
    }

    for feature in request.derived_features:
        input_features.update(
            feature.depending_on
        )

    for feature in request.aggregated_features:
        input_features.update(
            feature.derived_feature.depending_on
        )
    ...
    with st.form("Input Features"):
        input_df = st.data_editor(
            pd.DataFrame(
                columns=[
                    feat.name for feat in input_features
                ]
            ),
            num_rows="dynamic"
        )
    ...
    if need_event_timestamp:
        input_df[view.view.event_timestamp.name] = pd.to_datetime(input_df[view.view.event_timestamp.name])

    ensure_job = RetrivalJob.from_convertable(
        input_df,
        request
    ).ensure_types([ensure_types_req]).with_request([request])
...
async def model_get_input_features(model_store: ModelFeatureStore) -> None:
    ...
    if isinstance(model_store.model.features, FeatureInputVersions):
        ...
        selected_version = st.selectbox("Select Feature Version", keys, index=default_index, key="Select version for features")
        model_store = model_store.using_version(selected_version)

    needed_columns = [
        ent.name for ent 
        in model_store.needed_entities()
    ]
    ...
    with st.spinner("Loading..."):
        data = (await model_store.features_for(
            entities,
...
#     metric_values = {}
#
#     model_schema = store.model(model_name).model
#     metrics = await evaluation.evaluate(prediction, model_schema, store)
#
#     single_metrics: list[SingleMetric] = []
#     figures: list[FigureMetric] = []
#
#     for metric in metrics:
#         if isinstance(metric, SingleMetric):
#             single_metrics.append(metric)
#         elif isinstance(metric, FigureMetric):
#             figures.append(metric)
#
#
#     if single_metrics:
#         st.subheader("Metrics")
#         cols = st.columns(min(len(single_metrics), 3))
#         for index, metric in enumerate(single_metrics):
#             metric_values[metric.name] = metric.value
#
#             delta = None
#             if metric.name in baseline_metrics:
#                 delta = metric.value - baseline_metrics[metric.name]
...
#             col = cols[index % len(cols)]
#             col.write(figure.name)
#             col.pyplot(figure.figure) # type: ignore
#
...
#     target: pl.Series,
#     col: DeltaGenerator,
#     metrics: list[SingleMetricEvaluation] | None = None,
...
#                     y_true=target.to_pandas(), 
#                     y_pred=pred.to_pandas()
#                 ).plot(
#                     include_values=True, 
#                     cmap="viridis", 
#                     ax=None, 
...
#
#     if len(metrics) > 0:
#         col.subheader("Metrics")
#         cols = col.columns(min(len(metrics), 3))
#         for index, metric in enumerate(metrics):
#             value = metric.evaluate(prediction.to_pandas(), target.to_pandas())
...
#
#             if metric.name in baseline_metrics:
#                 delta = value - baseline_metrics[metric.name]
#                 delta = int(delta * 1000) / 1000
#
#             cols[index % len(cols)].metric(
#                 metric.name, 
...
#
#     if figures:
#         col.subheader("Figures")
#         cols = col.columns(min(len(figures), 3))
#         for index, (name, figure) in enumerate(figures):
#             sub_col = cols[index % len(cols)]
#             sub_col.write(name)
#             sub_col.pyplot(figure(prediction, target))
#
#     return metric_values
...
#     target: pl.Series,
#     metrics: list[SingleMetricEvaluation] | None = None,
#     figures: list[tuple[str, Callable[[pl.Series, pl.Series], Figure]]] | None = None
# ):
...
#             col = cols[index % len(cols)]
#             col.write(name)
#             col.pyplot(figure(prediction, target))
...
def mermiad_graph(nodes: set[GraphNode], edges: set[GraphEdge], graph_type: str = "LR") -> str:
    ...
    node_strings = ";\n".join([
        format_node(node) for node in nodes
    ])
...
async def manually_show_features(store: FeatureStore, view_name: str):
    ...
    features = (await view_store.features_for(
        entities,
        event_timestamp_column=event_timestamp
    ).log_each_job().to_polars()).collect()
...
async def load_feature_view_data(store: FeatureStore, view_name: str):
    ...
    with all_data:
        ...
        features = st.multiselect("Features", all_features, default=all_features, key=f"{view_name}-load_features_preview")

        if st.button("Load data", key="load_limit_preview"):
            start_time = datetime.now()
            with st.spinner("Loading..."):
                data = await view_store.select(set(features)).all(
                    limit=int(limit)
                ).log_each_job().to_polars()
...
async def view_freshness(store: FeatureStore, view_name: str, key: str = "load_freshness"):
    ...
    if not view.event_timestamp:
        st.write("An event timestamp is needed to compute the freshness. It can be set with the following code:")
        st.code("""from aligned import EventTimestamp, feature_view
    ...
    if st.button("Load Data Freshness", key=key):
        try:
            ...
            if freshness is None:
                ...
                st.metric(
                    "Valid features up to",
                    freshness.strftime("%Y-%m-%d %H:%M:%S")
                )
                st.metric(
                    "Updated ... ago",
                    str(delta)
...
def show_mermiad_graph(
    ...
    selected_type = col.selectbox("Graph Type", types, index=types.index(graph_type), key=f'{graph_name}_type')
    ...
    </script>""", scrolling=True, height=int(height_input))
...
def feature_view_dependencies(store: FeatureStore, view_name: str, col: DeltaGenerator) -> None:
    ...
    mermaid_edges: set[GraphEdge] = {
        GraphEdge(dep.identifier, f"feature_view:{view_name}")
        for dep in all_dependencies
    }

    if not all_dependencies:
        st.write("The view has no dependencies. Therefore it is most likely a staging / source view.")
        return
    ...
    nodes = {
        GraphNode(dep.name, dep.identifier, dep.location)
        for dep in all_dependencies
    }
...
async def update_materialized_source_for_view(store: FeatureStore, view_name: str) -> None:
    ...
    if not view.materialized_source:
        st.write("No materialized source is set. Therefore it is most likely a staging / source view.")
        return

    if not isinstance(view.materialized_source, (WritableFeatureSource, DataFileReference)):
        st.write("The materialized source is not writable. Therefore it is most likely a staging / source view.")
        return


    depends_on_view = view.source.depends_on()
    if depends_on_view:
        st.write(f"When updating the materialzed source will it load data from:")

        for dep in depends_on_view:
            feature_location(dep)


    all_data, between_data, incremental_data = st.tabs(["Update All Data", "Update Between Dates", "Incremental Update"])
    ...
    with between_data:
        st.write("Load all existing data between two dates, and upsert it into the materialized source.")
        ...
        if st.button("Update Materialized Source", key="update_between_materialized"):
            ...
            with st.spinner("Materializing..."):
                await view_store.upsert(
                    view_store.using_source(view.source)
    ...
    with incremental_data:
        st.write("Load all data since the last materialization, and upsert it into the materialized source.")
        ...
        if st.button("Update Materialized Source", key="incremental_materialized"):
            ...
            if not start_date:
                ...
                st.write(f"Found freshness '{formatted_start_date}'. Loading data from then until now.")
                start_time = datetime.now()
                with st.spinner("Materializing..."):
                    await view_store.upsert(
                        view_store.using_source(view.source)
...
async def show_feature_view(store: FeatureStore, view_name: str):
    ...
    with side_col:
        with st.expander("Source Dependencies"):
            feature_view_dependencies(store, view_name, st)

    with side_col:
        with st.expander("Feature Dependencies"):
    ...
    with st.container(border=True):
        data_freshness, update_source, load_data, explore_data, transform_features = st.tabs([
            "Data Freshness", "Materialize Source", "Load Data", "Explore Data", "Transform Features"
        ])
        with data_freshness:
...
#         all_models = model_register.all_models()
#
#     selected_model = st.sidebar.selectbox("Select Model", all_models, index=None)
#
#     if not selected_model:
#         st.warning("Need to select a model.")
#         return
#
#     st.header(selected_model)
#
#     with st.spinner("Loading model..."):
#         model = model_register.load(selected_model)
#
#     with st.spinner("Loading feature store"):
#         store = await FeatureStore.from_dir(".")
#
#     if not isinstance(model, ModelWrapper):
#         st.warning(f"Model is of type {type(model)}. Expected ModelWrapper.")
#         return
#
#     if model.model_contract not in store.models:
#         st.warning(f"Model contract **'{model.model_contract}'** not found in feature store.")
#         return
#
#     st.write(f"Selected model contract: {model.model_contract}")
#     model_store = store.model(model.model_contract)
#     predict_view = model_store.model.predictions_view
#
#     if model.contract_version:
#         st.info(f"Selected model contract version: {model.contract_version}")
#         model_store = model_store.using_version(model.contract_version)
#     else:
#         st.info("No model contract version selected. Using default version.")
#
#     input_request = model_store.needed_entities()
#
#     st.subheader("Input Entities")
#     st.write("Need to provide the following entities")
#     for entity in input_request:
#         feature_cell(entity)
#
#     stored_features = predict_view.features.copy()
#     if predict_view.model_version_column:
#         stored_features.remove(predict_view.model_version_column)
#
#     st.subheader("Prediction Result") 
#     pred_result = predict_view.request("predict").request_result
#     st.write("The model need to produce the following features")
#
...
#         raise ValueError(f"Predictions view have more than one feature. Unsure what to name the prediction. {predict_view.features}")
#
#
#     entity_selection = {
...
#     }
#
#     entity_method = st.selectbox("Select Entity Selection Method", list(entity_selection.keys()), index=None)
#
#     if not entity_method:
#         st.warning("Need to select an entity selection method.")
#         return
#
#     entities = await entity_selection[entity_method]
#
#     if entities is None:
#         st.warning("Need to select entities.")
#         return
#
...
#     model_id: str
# ):
#     prediction_feature = list(stored_features)[0]
#     stored_et_column = predict_view.event_timestamp
#     pred_result = predict_view.request("predict").request_result
#
#     # st.subheader("Entities")
...
#     st.write(f"Loaded {features.height} valid rows")
#
#
#     with st.spinner("Predicting..."):
#         sorted_features = sorted(job.request_result.feature_columns)
#         input_features = explode_input(
#             features.select(sorted_features),
#             list(job.request_result.features)
#         )
#         predictions = await model.predict(input_features.to_pandas())
#
#
#     with st.spinner("Setting features"):
#         with_pred = features.with_columns(
#             pl.Series(predictions).alias(prediction_feature.name),
#         )
#         if predict_view.model_version_column:
#             with_pred = with_pred.with_columns(
#                 pl.lit(model_id).alias(predict_view.model_version_column.name)
#             )
#
#         if stored_et_column and stored_et_column.name not in with_pred.columns:
#             with_pred = with_pred.with_columns(
#                 pl.lit(datetime.now()).alias(stored_et_column.name)
#             )
#
#     with_pred = with_pred.select(pred_result.all_returned_columns)
#
#     st.subheader("Predictions")
#     st.dataframe(with_pred.head())
#
#     if not model_store.model.predictions_view:
#         st.warning("No prediction source is set, so no predictions can be stored.")
#         return
#
#     if predict_view.model_version_column:
#         if st.button("Insert Predictions"):
#             with st.spinner("Storing..."):
#                 await model_store.insert_predictions(with_pred)
#
#     if st.button("Upsert Predictions"):
#         with st.spinner("Storing..."):
#             await model_store.upsert_predictions(with_pred)


async def select_entities_range(store: FeatureStore, needed_entities: set[Feature]) -> pl.DataFrame | None:
    ...
    all_returned_columns = set(view_store
        .view
        .request_all
        .request_result
    ...
    missing_columns = { 
        ent.name for ent 
        in needed_entities 
        if ent.name not in all_returned_columns
    }
    ...
    with st.spinner("Loading..."):
        entities = (await view_store.between_dates(start_date, end_date)
            .to_polars()
        ).select(entity_columns).collect()

    return entities

async def select_entities_from_view(store: FeatureStore, needed_entities: set[Feature]) -> pl.DataFrame | None:
    ...
    with st.spinner("Loading..."):
        entities = (await view_store.all(limit=int(limit))
            .to_polars()
        ).select(entity_columns).collect()
...
async def data_catalog(store: FeatureStore, definition: RepoDefinition | None = None):
    ...
    selection = st.sidebar.selectbox("View Models or Features", ["Model Contracts", "Features", "Models"], index=None)
    ...
    if selection == "Model Contracts":
        ...
        pass
        # await predict_on_model(LocalModelRegister(Path("models")))
    else:
    ...
    # if definition:
    #     st.sidebar.metric("Definitions Created At", definition.metadata.created_at.strftime("%Y-%m-%d %H:%M:%S"))
    #     st.sidebar.metric("Name", definition.metadata.name)
    #
    #     if definition.metadata.github_url:
    #         st.sidebar.write(f"Github: {definition.metadata.github_url}")
...
async def generate_schema(store: FeatureStore):
    ...
    defaults = dict(zip(
        reversed(function.__code__.co_varnames),
        reversed(function.__defaults__ or [])
    ))
    ...
    with st.form("Config Source"):
        ...
        for name, annotation in annotations.items():
            ...
            if removed_optional == "datetime":
                ...
                st.caption(name)
                df = st.data_editor(pd.DataFrame(columns=["key", "value"]), num_rows="dynamic", key=name)
                value = dict(
                    zip(
                        df["key"].tolist(),
                        df["value"].tolist()
