...
def assign_interval(value: int | float, intervals: list[tuple[int | float, int | float]]) -> str:
    """
...
def get_dummies(
    ...
    dummies = pd.get_dummies(
        data=df, columns=[ING_COLUMN, PRICE_CATEGORY_COLUMN, COOKING_TIME_COLUMN]  # Constraints columns
    )

    dummies_df = (
        df[[RECIPE_ID_COLUMN]]
        .set_index(RECIPE_ID_COLUMN)
        .join(dummies.drop([RATING_COLUMN, UNIVERSE_COLUMN], axis=1).set_index(RECIPE_ID_COLUMN))
    )

    out = dummies_df.groupby([RECIPE_ID_COLUMN]).sum()
    out = out.applymap(
        lambda x: 1 if x > 0 else x
    )  # To avoid data errors, for example duplicated taxonomies in a recipe, this way the same constraint is not count twice or more times.
...
def get_priorities(df_recipes: pd.DataFrame) -> List[List[int]]:
    ...
    -------
    List[List[int]]
        List of lists of recipe IDs, where the first list contains the priority recipes with isUniverse and the second list contains the priority recipes with ingredients.
    """
    ...
    for is_universe in (True, False):  # True is Priority
        ...
        recipes = list(df_universe[RECIPE_ID_COLUMN].unique())
...
def get_priority_df(
    df: pd.DataFrame, dist: Dict[str, Union[int, float]], recipes: List[List[int]], just_fill: bool = False
) -> pd.DataFrame:
    """
    ...
    dist (Dict[str, Union[int, float]]): A dictionary where keys are column names and values are weights used to calculate overlays.
    recipes (List[List[int]]): A list of lists, where each inner list contains indices representing a recipe.
    just_fill (bool): A flag indicating whether to use the entire DataFrame (True) or only the subset with the maximum number of overlays (False). Default is False.

    Returns:
    ----------
    DataFrame: A DataFrame containing the priority rows based on the given recipes and overlay calculations. If the input DataFrame is empty, returns an empty DataFrame.
    """
...
def update_dist(
    ...
    Returns:
        Tuple[Dict[str, float], pd.DataFrame]: A tuple containing the updated distribution dictionary and the modified wanted DataFrame.
    """
    ...
    for protein in dist.keys():
        # Calculate the new quantity by subtracting the sum of the quantities in df from the current quantity in dist
...
def get_message(
    ...
    """
    Generates a status message based on the completeness and availability of ingredients, prices, and cooking times for recipes.
    ...
    if not len(ings_not_full + prices_not_full + cooking_times_not_full) and not recipes_not_full:
        return (0, f"TAXONOMY {taxonomy_id} SUCCESS.")  # Return success message if all conditions are met
...
def get_output(
    status: str,
    msg: str,
    ings_out: List[Union[str, int]] = [],
    prices_out: List[Union[str, int]] = [],
    cooking_times_out: List[Union[str, int]] = [],
    recipes: List[Dict[str, str]] = [],
) -> Dict[str, Union[str, int, List[Union[str, int]], List[Dict[str, str]]]]:
    """
...
def generate_menu(
    num_recipes: int,
    rules: Dict[str, Union[int, List[Dict[str, Union[int, str]]]]],
    df_recipes: pd.DataFrame,
    final_df: pd.DataFrame,
) -> Dict[str, Union[int, str, List[Union[str, int]], List[Dict[str, str]]]]:
    """
    ...
    Returns:
        Dict[str, Union[int, str, List[Union[str, int]], List[Dict[str, str]]]]: A dictionary containing the generated menu recipes, the number of recipes that could not be fulfilled, and messages about the generation process.
    """

    taxonomy_id = rules[TAX_COLUMN]
    if len(df_recipes) < 1:
        return get_output(0, f"No recipes with taxonomy {taxonomy_id}.")
    # Parse input data
    ...
    PIM_data_excluded = exclude_recipes(df_recipes, min_average_rating)  # Exclude recipes based on rating.
    ...
    # Theses lists are used to keep information about constraints not found in the data, are then used to generate the output.
    ings_not_found = []
    prices_not_found = []
    cooking_times_not_found = []
    dist = (
        {}
    )  # Dist dictionary is used to store the constraints found in data. This dict is mutable and will change in the algorithm run.
    for price in prices:
        if price["price_category_id"] in PIM_data_excluded[PRICE_CATEGORY_COLUMN].unique():
            dist[f"{PRICE_CATEGORY_COLUMN}_{int(price['price_category_id'])}"] = price["quantity"]
    ...
    for cooking_time in cooking_times:
        if f"{cooking_time['time_from']}_{cooking_time['time_to']}" in PIM_data_excluded[COOKING_TIME_COLUMN].unique():
            dist[f"{COOKING_TIME_COLUMN}_{cooking_time['time_from']}_{cooking_time['time_to']}"] = cooking_time[
                "quantity"
            ]
        else:
    ...
    df_recipe_group = (
        PIM_data_excluded[[RECIPE_ID_COLUMN, RATING_COLUMN, PRICE_CATEGORY_COLUMN, UNIVERSE_COLUMN]]
    ...
    df_recipe_group[UNIVERSE_COLUMN] = df_recipe_group.index.map(bool_result)

    out = get_dummies(PIM_data_excluded, df_recipe_group)  # Create a dummie df with 0's and 1's.
    available_recipes = out.copy()

    recipes_ordered = get_priorities(PIM_data_excluded)  # List with recipes ordered by priority.

    mult_df = get_priority_df(available_recipes, dist, recipes_ordered)  # DF with priority recipes
    ...
    while len(final_df) < num_recipes and len(mult_df) > 0:
        ...
        final_df = pd.concat([final_df, row])  # Add to final DF
        dist, available_recipes = update_dist(
            dist, row, available_recipes
    ...
    while len(final_df) < num_recipes and len(available_recipes) > 0:
        ...
        available_recipes = available_recipes.drop(remaining_df_add.index)
    ...
    msg_recipes = (
        f" Number of Recipes needed to fufill constraints is {len(final_df) - random_recipes}, adding {random_recipes} random recipes."
        if random_recipes
        else ""
    )

    ings_out, ings_not_full = get_ingredient_distribution(ingredients, final_df, ings_not_found)

    prices_out, prices_not_full = get_prices_distribution(prices, final_df, prices_not_found)

    cooking_times_out, cooking_times_not_full = get_cooking_times_distribution(
        cooking_times, final_df, cooking_times_not_found
    )
    ...
    final_recipes = df_recipes[df_recipes[RECIPE_ID_COLUMN].isin(final_df.index)][
        [RECIPE_ID_COLUMN, ING_COLUMN]
    ].drop_duplicates()

    final_recipes = final_recipes.merge(final_df[[CONSTRAINT_COLUMN]].reset_index(), on=RECIPE_ID_COLUMN, how="inner")

    recipes = final_recipes.to_dict(orient="records")

    return get_output(status, msg, ings_out, prices_out, cooking_times_out, recipes)
...
def generate_menu_companies_sous_chef(
    week: int, year: int, companies: List[Dict[str, Any]], input_df: Optional[pd.DataFrame] = None
) -> Dict[str, Any]:
    """
    ...
    Args:
        ...
        input_df (Optional[pd.DataFrame]): DataFrame containing recipe data.
