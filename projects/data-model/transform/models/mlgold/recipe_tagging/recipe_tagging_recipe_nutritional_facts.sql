...
, nutrition_calculation as (
    select
        ...
        , sum(
            ...
            * coalesce(nutrition_units, 0)
            * case
                when nutrient_id = '{{ var("nutrient_id_fresh_fruit_veg") }}' then nutrient_value else 0
            end
        ) as fg_fresh_g
        , sum(
            (coalesce(net_weight, -1) / 100.0)
            * coalesce(nutrition_units, 0)
            * case
                when nutrient_id = '{{ var("nutrient_id_processed_fruit_veg") }}' then nutrient_value else 0
            end
        ) as fg_proc_g
