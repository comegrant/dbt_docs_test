...
update_subscription_step_viewed_godtlevert as (
    ...
    select * from {{ ref('base_segment_javascript_godtlevert__update_subscription_step_viewed') }}

)

, update_subscription_step_viewed_adams as (

    select * from {{ ref('base_segment_javascript_adams__update_subscription_step_viewed') }}

)

, update_subscription_step_viewed_linas as (

    select * from {{ ref('base_segment_javascript_linas__update_subscription_step_viewed') }}

)

, update_subscription_step_viewed_retnemt as (

    select * from {{ ref('base_segment_javascript_retnemt__update_subscription_step_viewed') }}
