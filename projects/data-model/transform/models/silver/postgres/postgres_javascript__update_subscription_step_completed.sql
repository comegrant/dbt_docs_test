...
update_subscription_step_completed_godtlevert as (
    ...
    select * from {{ ref('base_postgres_javascript_godtlevert__update_subscription_step_completed') }}

)

, update_subscription_step_completed_adams as (

    select * from {{ ref('base_postgres_javascript_adams__update_subscription_step_completed') }}

)

, update_subscription_step_completed_linas as (

    select * from {{ ref('base_postgres_javascript_linas__update_subscription_step_completed') }}

)

, update_subscription_step_completed_retnemt as (

    select * from {{ ref('base_postgres_javascript_retnemt__update_subscription_step_completed') }}
