...
, attribute_values_pivot as (
    ...
    -- exclude the attributes that we are not interested in
    inner join attribute_value_templates_filtered
        on attribute_value_templates_filtered.attribute_id = attribute_values.attribute_id
    group by product_variation_id, company_id

)

, attribute_values_clean as (
    select
        product_variation_id
        , company_id
    ...
    from attribute_values_pivot
)

select * from attribute_values_clean
