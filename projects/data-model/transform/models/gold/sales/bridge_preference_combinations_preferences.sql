...
, joined as (
    ...
    select
    md5(concat(
        cast(preference_combinations.pk_dim_preference_combinations as string)
        , cast(preferences.preference_id as string)
        , cast(company_id as string)
        )
    ) as pk_bridge_preference_combinations_preferences
    , preference_combinations.pk_dim_preference_combinations as fk_dim_preference_combinations
